...
type Round={id:string;round_number:number;status:'upcoming'|'open'|'locked'|'finished';predictions_revealed?:boolean;deadline_at?:string|null}
...
type SubmissionRow={match_id:string;player_id:string}
type Outcome='H'|'D'|'A'

const fotmob=(id:number)=>`https://images.fotmob.com/image_resources/logo/teamlogo/${id}.png`
const logoByTeam:Record<string,string>={
  'локомотив':'https://logotyp.us/file/lokomotiv-moscow.svg','локомотив москва':'https://logotyp.us/file/lokomotiv-moscow.svg','локо':'https://logotyp.us/file/lokomotiv-moscow.svg','зенит':'https://logotyp.us/file/zenit.svg','динамо':'https://logotyp.us/file/dynamo.svg','динамо москва':'https://logotyp.us/file/dynamo.svg','динамо м':'https://logotyp.us/file/dynamo.svg','оренбург':'https://logotyp.us/file/orenburg.svg','краснодар':'https://logotyp.us/file/krasnodar.svg','рубин':'https://logotyp.us/file/rubin-kazan.svg','рубин казань':'https://logotyp.us/file/rubin-kazan.svg','цска':'https://logotyp.us/file/cska-moscow.svg','цска москва':'https://logotyp.us/file/cska-moscow.svg','ростов':'https://logotyp.us/file/rostov.svg','балтика':'https://assets.football-logos.cc/logos/russia/1500x1500/baltika.b4c4f0b4.png','ахмат':fotmob(8708),'ахмат грозный':fotmob(8708),'крылья советов':fotmob(8709),'крылья':fotmob(8709),'крылья советов самара':fotmob(8709),'акрон':fotmob(1068364),'акрон тольятти':fotmob(1068364),'факел':fotmob(1692),'факел воронеж':fotmob(1692),'динамо махачкала':fotmob(1068353),'динамо мх':fotmob(1068353),'динамо (махачкала)':fotmob(1068353),'родина':fotmob(1066681),'родина москва':fotmob(1066681),
}
...
function TeamBadge({name,size='normal'}:{name:string;size?:'small'|'normal'}){const key=teamKey(name),src=logoByTeam[key];const[failed,setFailed]=useState(false);if(key==='спартак'||key==='спартак москва')return <span className={`teamEmoji ${size}`} role="img" aria-label="Спартак">🐷</span>;if(!src||failed)return <span className={`teamFallback ${size}`}>{initials(name)}</span>;return <img className={`teamLogo ${size}`} src={src} alt={`Логотип ${name}`} loading="lazy" onError={()=>setFailed(true)}/>}
function TeamName({name,side}:{name:string;side:'home'|'away'}){return <div className={`teamName ${side}`}><TeamBadge name={name}/><strong>{name}</strong></div>}
async function api<T>(path:string):Promise<T>{const res=await fetch(`${SUPABASE_URL}/rest/v1/${path}`,{headers,cache:'no-store'});const text=await res.text();if(!res.ok)throw new Error(text||`HTTP ${res.status}`);return(text?JSON.parse(text):undefined)as T}
function score(m:Match):Score|undefined{return m.home_score===null||m.away_score===null?undefined:{home:m.home_score,away:m.away_score}}
function inQuery(ids:string[]){return ids.length?`in.(${ids.join(',')})`:'eq.__none__'}
function outcome(s:Score):Outcome{return s.home>s.away?'H':s.home<s.away?'A':'D'}
function pointsForSide(s:Score,side:'home'|'away'){const o=outcome(s);if(o==='D')return 1;return (o==='H'&&side==='home')||(o==='A'&&side==='away')?3:0}
function joinNames<T extends {name:string}>(xs:T[]){return xs.map(x=>x.name).join(' · ')||'—'}

export default function HomePage(){
  const[tab,setTab]=useState<Tab>('round'),[players,setPlayers]=useState<Player[]>([]),[rounds,setRounds]=useState<Round[]>([]),[matches,setMatches]=useState<Match[]>([]),[predictionRows,setPredictionRows]=useState<PredictionRow[]>([]),[submissionRows,setSubmissionRows]=useState<SubmissionRow[]>([])
  const[roundNumber,setRoundNumber]=useState(4),[loading,setLoading]=useState(true),[message,setMessage]=useState('')

  const loadData=useCallback(async()=>{try{
    const[p,r,m]=await Promise.all([
      api<Player[]>('players?select=id,name,sort_order&order=sort_order.asc'),
      api<Round[]>('rounds?select=id,round_number,status,predictions_revealed,deadline_at&order=round_number.asc'),
      api<Match[]>('matches?select=id,round_id,home_team,away_team,home_score,away_score&order=created_at.asc')
    ])
    setPlayers(p);setRounds(r);setMatches(m)
    const current=r.find(x=>x.status==='open')||r[r.length-1];if(current)setRoundNumber(current.round_number)
    const visibleRoundIds=new Set(r.filter(x=>x.status==='finished'||x.status==='locked'||x.predictions_revealed).map(x=>x.id))
    const visibleMatchIds=m.filter(x=>visibleRoundIds.has(x.round_id)).map(x=>x.id)
    const currentMatchIds=current?m.filter(x=>x.round_id===current.id).map(x=>x.id):[]
    const[visiblePreds,submissions]=await Promise.all([
      visibleMatchIds.length?api<PredictionRow[]>(`predictions?select=match_id,player_id,home_score,away_score&match_id=${inQuery(visibleMatchIds)}`):Promise.resolve([]),
      currentMatchIds.length?api<SubmissionRow[]>(`predictions?select=match_id,player_id&match_id=${inQuery(currentMatchIds)}`):Promise.resolve([])
    ])
    setPredictionRows(visiblePreds);setSubmissionRows(submissions)
  }catch(e){console.error(e);setMessage('Не удалось загрузить данные')}finally{setLoading(false)}},[])
  useEffect(()=>{loadData()},[loadData])

  const selectedRound=rounds.find(r=>r.round_number===roundNumber),isCurrent=selectedRound?.status==='open',revealed=!!selectedRound&&(selectedRound.status==='finished'||selectedRound.status==='locked'||!!selectedRound.predictions_revealed),roundMatches=matches.filter(m=>m.round_id===selectedRound?.id)
  const getPrediction=(pid:string,matchId:string):Score|undefined=>{const row=predictionRows.find(p=>p.player_id===pid&&p.match_id===matchId);return row?{home:row.home_score,away:row.away_score}:undefined}
  const submissionCount=(pid:string)=>{const ids=new Set(roundMatches.map(m=>m.id));return submissionRows.filter(x=>x.player_id===pid&&ids.has(x.match_id)).length}

  const standings=useMemo(()=>players.map(player=>{let points=0,exact=0,outcomes=0,made=0;for(const m of matches){const actual=score(m);if(!actual)continue;const row=predictionRows.find(p=>p.player_id===player.id&&p.match_id===m.id);if(!row)continue;made++;const n=calculatePoints({home:row.home_score,away:row.away_score},actual);points+=n;if(n===3)exact++;if(n>=1)outcomes++}return{...player,points,exact,outcomes,made,average:made?points/made:0}}).sort((a,b)=>b.points-a.points||b.exact-a.exact||b.outcomes-a.outcomes),[players,matches,predictionRows])

  const leagueStats=useMemo(()=>{
    const finished=rounds.filter(r=>r.status==='finished').sort((a,b)=>a.round_number-b.round_number),wins=new Map<string,number>()
    const roundData=finished.map(r=>{const rms=matches.filter(m=>m.round_id===r.id&&score(m));const scores=players.map(p=>{let points=0;for(const m of rms){const pr=predictionRows.find(x=>x.player_id===p.id&&x.match_id===m.id),actual=score(m);if(pr&&actual)points+=calculatePoints({home:pr.home_score,away:pr.away_score},actual)}return{id:p.id,name:p.name,points}});const top=Math.max(0,...scores.map(x=>x.points));const heroes=scores.filter(x=>x.points===top&&rms.length>0);heroes.forEach(h=>wins.set(h.id,(wins.get(h.id)||0)+1));return{round:r.round_number,heroes,top}})
    const latest=roundData[roundData.length-1]
    const streaks=players.map(p=>{let current=0,best=0;for(const m of matches){const actual=score(m);if(!actual)continue;const pr=predictionRows.find(x=>x.player_id===p.id&&x.match_id===m.id);if(!pr)continue;const n=calculatePoints({home:pr.home_score,away:pr.away_score},actual);if(n>0){current++;best=Math.max(best,current)}else current=0}return{id:p.id,name:p.name,current,best}})
    const maxExact=Math.max(0,...standings.map(s=>s.exact)),snipers=standings.filter(s=>s.exact===maxExact&&maxExact>0),maxBest=Math.max(0,...streaks.map(s=>s.best)),streakKings=streaks.filter(s=>s.best===maxBest&&maxBest>0),maxWins=Math.max(0,...Array.from(wins.values())),roundKings=players.filter(p=>(wins.get(p.id)||0)===maxWins&&maxWins>0)
    return{latest,snipers,maxExact,streakKings,maxBest,roundKings,maxWins}
  },[players,rounds,matches,predictionRows,standings])

  const analytics=useMemo(()=>{
    const finishedIds=new Set(rounds.filter(r=>r.status==='finished').map(r=>r.id))
  ...
  const leader=standings[0]?.points??0,completedMatches=matches.filter(m=>score(m)).length
